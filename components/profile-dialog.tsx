              ...
              <section className="rounded-[26px] border border-slate-200/80 bg-white/80 p-4 shadow-sm dark:border-slate-800/80 dark:bg-slate-950/45">
                ...
                <div className="grid gap-3 md:grid-cols-3">
                  {primaryMetrics.map((field) => (
                    <div key={field.id} className="rounded-[22px] border border-slate-200/80 bg-slate-50/85 p-3 dark:border-slate-800/80 dark:bg-slate-900/75">
                      ...
                      <div className="mt-2 flex items-end justify-between gap-2">
                        <Input
                          ...
                          className="h-10 border-0 bg-transparent px-0 text-[1.6rem] font-semibold tracking-tight text-slate-900 shadow-none focus-visible:ring-0 dark:text-white"
                        />
                        {field.suffix ? <span className="pb-1.5 text-[11px] font-medium uppercase tracking-[0.14em] text-slate-400">{field.suffix}</span> : null}
                      </div>
                    </div>
                  ))}
                </div>
                    ...
                    <div key={field.id} className="rounded-[22px] border border-white/70 bg-white/70 p-3 dark:border-white/5 dark:bg-slate-950/35">
                      ...
                      <div className="mt-2 flex items-end justify-between gap-2">
                        <Input
                          ...
                          value={field.value}
                          onChange={(event) => field.onChange(event.target.value)}
                          className="h-10 border-0 bg-transparent px-0 text-[1.4rem] font-semibold tracking-tight text-slate-900 shadow-none focus-visible:ring-0 dark:text-white"
                        />
                        {field.suffix ? <span className="pb-1.5 text-[10px] font-medium uppercase tracking-[0.14em] text-slate-400">{field.suffix}</span> : null}
                      </div>
                    </div>
                  ))}
                </div>
                ...
                {recommendedPlan ? (
                  <div className="grid h-full gap-3 lg:grid-cols-[minmax(0,1fr)_200px]">
                    ...
                    <div className="grid content-start gap-3">
                      ...
                      <Button asChild className="h-10 rounded-2xl bg-gradient-to-r from-orange-500 to-rose-500 text-sm hover:from-orange-600 hover:to-rose-600">
                        <Link href="/workouts/generate">Usar esta propuesta</Link>
                      </Button>
                    </div>
                  </div>
                ) : (
                  <div className="flex h-full items-center rounded-[22px] border border-dashed border-orange-200/80 bg-orange-50/55 p-4 text-sm leading-6 text-slate-600 dark:border-orange-400/10 dark:bg-orange-500/5 dark:text-slate-400">
              ...
              <div className="grid gap-3">
                ...
                <div className="flex flex-col justify-between rounded-[26px] border border-slate-200/80 bg-[linear-gradient(160deg,rgba(255,255,255,0.9),rgba(241,245,249,0.9))] p-4 shadow-sm dark:border-slate-800/80 dark:bg-[linear-gradient(160deg,rgba(15,23,42,0.88),rgba(30,41,59,0.7))]">
                  ...
                  <div className="mt-4 flex gap-3">
                    <Button variant="outline" className="h-10 flex-1 rounded-2xl text-sm" onClick={() => setOpen(false)}>
                      Cancelar
                    </Button>
                    <Button className="h-10 flex-1 rounded-2xl bg-gradient-to-r from-orange-500 to-rose-500 text-sm hover:from-orange-600 hover:to-rose-600" onClick={handleSave}>
                      Guardar
                    </Button>
                  </div>
                </div>
